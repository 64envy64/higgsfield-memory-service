# Memory Service HTTP Contract

> Read alongside [`BUILD_SPEC.md`](BUILD_SPEC.md). This file is the **task as given**; the build spec is **how we decided to answer it** (plan + argue deltas).
...
A turn may contain one or more messages. Multi-message turns (including tool calls) are valid. Handle both gracefully.
...
Return all stored memories for a user. **Used for debugging and inspection during review.**
...
These are the problems that separate thoughtful implementations from boilerplate.
...
* **LLM usage is encouraged.** Use any LLM you want for extraction, summarization, or retrieval. Document which models you use and why in the README. List required API keys in `.env.example`.
...
**The CHANGELOG should show the engineering process. A service with a thoughtful 5-entry CHANGELOG is easier to trust than a higher-scoring black box with no iteration history.**
...
**This is the iteration loop.** Building a quality fixture early and running it after every change produces measurably better systems.

### We provide: a smoke test

To verify your service is compatible with the eval harness:
...
* README explains the design in 5 minutes.
...
You can read public memory-system designs (mem0, hindsight, honcho, mnemonic agents, etc.) for inspiration. **The service must be its own design.** Don't lift another project's API shape or recall pipeline. If the work closely resembles an existing system, the resemblance should be explainable line by line.

The goal is to design a memory system, not rename an existing one.
